...
public class sex_LGMilitarySubmarketPlugin extends BaseSubmarketPlugin {
	...
	public sex_LGMilitarySubmarketPlugin() {
	}
	...
	public void updateCargoPrePlayerInteraction() {
		...
		if (this.okToUpdateShipsAndWeapons()) {
			...
			try {
				JSONObject stingerData =Global.getSettings().getMergedJSONForMod("data/config/sex_lions_guard_market.json","hakkari2");
			...
			for(int i=0;i<ships;i++){
				...
				if(hull!=null){
					FleetMemberAPI ship =this.addShip(hullPicker.pick().getHullId()+"_Hull", true, 1f - randomReduction);
	...
	public boolean isEnabled(CoreUIAPI ui) {
		...
		if (commissionFactionId != null && commissionFactionId.equals(Factions.DIKTAT)) {
	...
	public String getTooltipAppendix(CoreUIAPI ui) {
		// Check if the player has the required commission with the specified faction
		boolean hasRequiredCommission = "diktat".equals(Misc.getCommissionFactionId());

		if (!hasRequiredCommission) {
			return "Requires a commission with the Sindrian Diktat.";
		}
	...
	public String getIllegalTransferText(CargoStackAPI stack, SubmarketPlugin.TransferAction action) {
		return "The Vanguard does not do buybacks or refunds.";
	}
	...
	public String getIllegalTransferText(FleetMemberAPI member, SubmarketPlugin.TransferAction action) {
		return "The Vanguard Branch does not do buybacks or refunds.";
	}
	public Highlights getTooltipAppendixHighlights(CoreUIAPI ui) {
		...
		if (!hasRequiredCommission) {
			...
			p.setText("Requires a commission with the Sindrian Diktat.");
		...
		if (!isEnabled(ui)) {
			...
			j.setText("Requires a commission with the Sindrian Diktat.");
