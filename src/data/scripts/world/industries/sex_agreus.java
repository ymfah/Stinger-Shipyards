...
public class sex_agreus extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {

	@Override
	public boolean isHidden() {
		if (!market.getFactionId().equals(Factions.INDEPENDENT) && !Global.getSector().getMemoryWithoutUpdate().getBoolean("$sex_AgreusHidden")) {
			return true;
		}
		return false;
	}
...
//	}
	...
	public void advance(float amount) {
		if (!market.getFactionId().equals(Factions.INDEPENDENT) && !Global.getSector().getMemoryWithoutUpdate().getBoolean("$sex_AgreusRemoved")) {
			market.addIndustry(Industries.PATROLHQ);
			Global.getSector().getMemoryWithoutUpdate().set("$sex_AgreusHidden", true);
		}
...
//			}
			...
			float patrolDays = 70f + (float) Math.random() * 10f;
...
//		return route.getActiveFleet() != null && route.getActiveFleet().getFleetPoints() >= custom.spawnFP * 0.6f;
	...
	public static CampaignFleetAPI createPatrol(PatrolType type, String factionId, RouteData route, MarketAPI market, Vector2f locInHyper, Random random) {
		...
		fleet.setName("Free Star Combine Regulars");
