...
public class sex_command extends BaseIndustry implements RouteFleetSpawner, FleetEventListener {
	...
	public void apply() {
		...
		if (isFunctional()) {
			if (!market.hasSubmarket("sex_merc_market") && market.getFaction().getId().equals("sex_co") && !market.isPlayerOwned()) {
				market.addSubmarket("sex_merc_market");
				SubmarketAPI sub = market.getSubmarket("sex_merc_market");
				sub.setFaction(Global.getSector().getFaction("sex_co"));
				Global.getSector().getEconomy().forceStockpileUpdate(market);
			}
		} else {
			market.removeSubmarket("sex_merc_market");
		}
		if (!isFunctional()) {
			unapply();
		}
	...
	public static int getPatrolCombatFP(PatrolType type, Random random) {
		...
		switch (type) {
			...
			case COMBAT:
				...
				break;
			case HEAVY:
				combat = Math.round(12f + (float) random.nextFloat() * 6f) * 6f;
				break;
		}
